...
def split(dataset,downsampling,screw):
    ...
    val['X'] = failed['X'][train_failed_len:val_failed_len] + healthy['X'][train_healthy_len:val_healthy_len]
    val['Y'] = failed['Y'][train_failed_len:val_failed_len] + healthy['Y'][train_healthy_len:val_healthy_len]
    ...
    if screw:
        ...
        if genefile is not None:
            file = open(genefile,'rb')
            generated_data = pickle.load(file)
            train['X'].extend(generated_data['X'])
            generated_data['Y'] = list(generated_data['Y'])   
            train['Y'].extend(generated_data['Y'])
...
def varify(disk_model,path,genepath,genename):
    global drop_index, genefile
    drop_index = [int(i) for i in config_parse(disk_model,'drop_index')]
    if genename is None:
        genefile = None
    else:
        genefile = os.path.join(genepath,genename)
    file = open(os.path.join(path,disk_model,'dataset.pkl'),'rb')
    ...
    model.fit(train['X'],train['Y'],batch_size=BATCHSIZE,epochs=EPOCHS,validation_data=(val['X'],val['Y']),verbose=False)
...
def main():
    ...
    disk_model = 'Disk_1'
    genepath = './'
    genename = 'generated_mixup.pkl'
    varify(disk_model,datapath,genepath,genename)
