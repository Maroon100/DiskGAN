...
from TimeGAN.time_dataset import TimeDataset, batch_generator
from TimeGAN.visualization_metrics import visualization
import argparse
import os
import pickle
...
def gen(dataset, device, generator, supervisor, recovery, disk_model, savepath, gen_num):
    ...
    generated_data = generated_data[:gen_num]
    labels = [[1]]*gen_num
    generated = {'X':generated_data,'Y':labels}
    file = open(os.path.join(savepath,disk_model+'_timegan.pkl'),'wb')
...
def generate(disk_model,path,savepath,gen_num):
    ...
    parser.add_argument("--data-path", type=str, default=os.path.join(path,disk_model,filename))
    ...
    dataset = TimeDataset(args.data_path)
    input_size = dataset[0].shape[1]
    hidden_size = 24
    num_layers = 3
    ...
    gen(dataset, device, generator, supervisor, recovery, disk_model, savepath, gen_num)
