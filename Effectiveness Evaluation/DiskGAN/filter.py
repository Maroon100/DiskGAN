...
def train(disk_model,path,savepath):
    ...
    if FAR<FAR_THRESHOLD:              # As we only care the FAR, thus we save the model when the FAR is lower than a predefined threshold. Note that the threshold is a hyperparameter and maybe different for different disk models.
        model.save(os.path.join(savepath,disk_model+'_filter.h5'))
        print("The filter has been successfully saved !!!")    
...
def get_filter():
    ...
    train(disk_model,path,savepath)
