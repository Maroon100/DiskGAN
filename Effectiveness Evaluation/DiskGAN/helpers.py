...
def prepare_discriminator_data_wgan(pos_samples, neg_samples, gpu=False):

    inp_r = torch.tensor(pos_samples,torch.LongTensor)
    inp_f = torch.tensor(neg_samples,torch.LongTensor)

    perm_r = torch.randperm(target_r.size()[0])
    inp_r = inp_r[perm_r]

    perm_f = torch.randperm(target_f.size()[0])
    inp_f = inp_f[perm_f]

    inp_r = Variable(pos_samples)
    inp_f = Variable(neg_samples)
